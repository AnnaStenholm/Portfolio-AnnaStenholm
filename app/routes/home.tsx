...
export function meta({}: Route.MetaArgs) {
  return [
    { title: "Anna Stenholm - Frontend Developer & UX Designer" },
    { name: "description", content: "Frontend developer and UX designer specializing in React and JavaScript with a passion for smooth, responsive user flows." },
  ];
}
      ...
      <div className="flex-1 flex flex-col items-center gap-16 min-h-0">
        ...
        <div className="max-w-[1244px] w-full space-y-6 px-4">
          ...
          <div className="flex grid-rows-2 gap-6 md:grid-cols-2 md:grid">
            <div>
              <ul className="mt-6">
                {location.map(({  text, icon }) => (
                  <li key={text}>
                    <h3 className="flex items-center p-1  text-slate-700  dark:text-slate-700" rel="noreferrer">
                      {icon}
                      {text}
                    </h3>
                  </li>
            ...
            <div>
              <img src={img} alt="Anna Stenholm" className="w-48 h-48 rounded-full mx-auto" />
            </div>
          </div>
        </div>
